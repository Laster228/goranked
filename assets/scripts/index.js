...
const selectInit = () => {
    ...
    if (selectArray.length > 0) {
        selectArray.forEach((element, index) => {
            ...
            element.style.setProperty('--bg-name', `url('../images/ranks/${selectTag.value}.webp')`);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(heroesBlock){
        ...
        if(heroesGridArray.length){
            const createBlock = (heroName) =>{
                ...
                heroesImg.setAttribute('src',`${__path}/${heroName}.webp`);
    ...
    // range function TODO: rewrite in one class
    ...
    if (rankItemArray.length) {
        rankItemArray.forEach(rankItem => {
            ...
            rankItemLabel.style.setProperty('--bg-name', `url('../images/ranks/${newRank}.webp')`);
